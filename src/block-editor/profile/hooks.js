...
export const useProfileId = ( props, pid = null ) => {
	...
	const { postId: selectedProfileId = null } = attributes;

	const { currentPostType, currentPostId } = useSelect(
		( select ) => {
			return {
	...
	const isProfilePage =
		currentPostType === PROFILE_POST_TYPE &&
	...
	if ( isProfilePage ) {
		...
		derivedProfileId = isDirectSelection
			? selectedProfileId
			: inheritedSelectedProfileId;
	}
...
export const useProfile = ( profileId ) => {
	const query = useSelect(
		( select ) => {
			...
			const hasStartedResolution = select(
				coreDataStore
			...
			const hasFinishedResolution = select(
				coreDataStore
			).hasFinishedResolution( 'getEntityRecord', selectorArgs );
			const hasResolutionFailed = select(
				coreDataStore
			).hasResolutionFailed( 'getEntityRecord', selectorArgs );

			return {
				profile: select( coreDataStore ).getEntityRecord(
					...selectorArgs
				),
