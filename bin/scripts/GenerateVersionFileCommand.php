...
class GenerateVersionFileCommand extends Command {
	...
	private function getPluginFileHeaders() {
		...
		$file_data = file_get_contents( $file, false, null, 0, 8 * 1024 ); //phpcs:ignore WordPressVIPMinimum.Performance.FetchingRemoteData.FileGetContentsUnknown
	...
	private function getVersion( InputInterface $input, OutputInterface $output, ): string {
		...
		} elseif ( ! $input->getOption( 'major' ) || ! $input->getOption( 'minor' ) || ! $input->getOption( 'minor' ) ) {
	...
	private function generateCode( $version ): string {
		...
		$code = sprintf( "<?php  \r\n return %s;", var_export( $version, true ) ); //phpcs:ignore WordPress.PHP.DevelopmentFunctions
		return $code;
	}
